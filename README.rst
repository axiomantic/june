...
``tools/generate_compile_harness.py`` and CALLS the bound methods no
behavioural test reaches. Not all of them: the generator prints what it emitted
and what it skipped, with a reason for each - a setter the field check covers,
a free function or a method with no receiver for which it has no call form, an
operator, a type it does not export. It asserts nothing. Its whole job is to
